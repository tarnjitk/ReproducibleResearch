...
The 5-minute interval, on average across all the days in the dataset, which contains the maximum number of steps is at `r activityTimeSeries$Time[maxStepsInt]` in the morning.
...
There are a total of `r numOfMissing ` missing values in the dataset (i.e. the total number of rows with NAs) specifically in the $STEPS$ column. To handle these missing values in the dataset, they are replaced by the mean number of steps of the complete dataset.
